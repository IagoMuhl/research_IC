program quant_HGammafix
   ...
   real*8:: T_inicial,T_final, Alfa, Alfa2, Gamma, erro,Z2,F
   ...
   if(state/='2AF') then
      ...
      do while (T_inicial/=T_final) !FUNÇÃO DE PARTIÇÃO/ LOOP TEMPERATURA
         ...
         do while(erro >= tol)
            ...
            call magnetization_diag(W,Z,T_inicial,dim,sigma_z,V,m1)
            call magnetization_diag(W2,Z2,T_inicial,dim,sigma_z,V2,m2)
