...
!       CALL CPU_TIME ( tempo_inicial )
     ...
     !    end if
         ...
         !open(unit=34, file='08_PM(T)_j1+0.6_jx+0.463_T=0', access='stream', form='formatted')


         WRITE (temp, '(F6.3)') T

         open(unit=20, file= trim(temp)//'_SO_H_'//trim(state)//'_T-H.dat', access='stream', form='formatted')
         ...
         do while (H_inicial/=H_final)
            ...
            !write(20,*) H_inicial, m_order, S_ns
