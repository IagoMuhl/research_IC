program square_T
   ...
   real*8:: T,H,step,Z,Z_prime,m_order,m_second,tol,F,H_max,F_prime
   ...
   tol = 10.d0**(-8); J2 = 1.0d0; J3 = -0.1d0; s_z = 0; T = 10.d0**(-3)
!----------------------------BASE-------------------------------
   call base(s)
...
!--------------------------------------------------------------
   ...
   H = 0.1; H_max = 5; cd = -3; step = 10.d0**(cd)
   ...
   !do
         ...
         open(unit=20, file= 'Temp_' // trim(temp) // '_Var-Fase_' // trim(state) // "_H-T.dat")
         ...
         do while (H/=H_max)
            ...
            write(20,*) H, F, m_order, m_second, mag
...
!             end if
            ...
            if ((max(H,H_max))-(min(H,H_max))<=abs(step)) then
               H = H_max
            endif
