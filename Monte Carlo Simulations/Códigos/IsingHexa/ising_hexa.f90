...
subroutine calculate_energy(s, L, E, J_1)
  ...
  do i = 1, L
     do j = 1, L
        ...
        ! Interação horizontal (J1)
        E = E + s(i,j) * s(i1,j) 
        
         if (mod(i, 2) == 1) then
         if (mod(j, 2) == 0) then
        ! Interação vertical (J1)
        E = E + s(i,j) * s(i,j1)
         endif
         endif
        ...
        !Interação de segundos vizinhos (J2)
         E = E + s(i,j) * s(ii,j1) 

         !Interação de terceiros vizinhos (J3)
         if (mod(i, 2) == 0) then
         if (mod(j, 2) == 1) then
        ! Interação vertical (J1)
        E = E + s(i,j) * s(i,j1)
